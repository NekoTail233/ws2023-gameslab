...
namespace VacuumBreather
{
    ...
    /// </summary>
    public class PIDRotate : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            {
                DesiredOrientation = GoalTransform.rotation;
            }

            if (GoalTransform != null && look)
            {
                Vector3 f = (GoalTransform.position - transform.position).normalized;
                Vector3 b = Vector3.Cross(f, Vector3.up);
                Vector3 u = Vector3.Cross(b, f);
                DesiredOrientation = Quaternion.LookRotation (f, u);
            }
